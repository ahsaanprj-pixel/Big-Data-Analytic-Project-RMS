...
class ConfigWindow(tk.Toplevel):
    ...
    def init_database(self):
        ...
        # ✅ Week 4 addition: Read and verify connection
        try:
            load_query = """SELECT * FROM gen_config"""
            res = self.fac_db.read_val(load_query)
            print("✅ Database connected successfully. Records found:")
            for row in res:
                print("   →", row)
    ...
    def save_fac_config(self):
        ...
        if fac_name and table_num and seat_num:
            ...
            self.check_if_empty_database()
            self.check_if_empty_fc_entry()
            messagebox.showinfo("Success", "Facility configuration saved successfully!")
        else:
            messagebox.showerror(
                "Empty input fields",
